...
export function createApiApp() {
      ...
      if (thesisId && collabList.length > 0) {
        for (const c of collabList) {
          ...
          if (collabUserId && collabUserId !== submitted_by) {
            ...
            if (insErr && process.env.NODE_ENV !== "production") console.error("[collaboration_requests insert]", insErr);
          }
        }
      }
  ...
  app.get("/api/my-submissions/:userId", async (req, res) => {
    try {
      ...
      if (collabRes.error && process.env.NODE_ENV !== "production") console.error("[my-submissions collaboration_requests]", collabRes.error);
